...
function createBaseCommerce(): Commerce {
  return { id: 0, fractionalize: false, access: false, sell: false, creator: "" };
}

export const Commerce = {
  encode(message: Commerce, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== 0) {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): Commerce {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCommerce();
  ...
  toJSON(message: Commerce): unknown {
    const obj: any = {};
  ...
  fromPartial<I extends Exact<DeepPartial<Commerce>, I>>(object: I): Commerce {
    const message = createBaseCommerce();
